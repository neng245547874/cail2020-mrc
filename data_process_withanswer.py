...
class InputFeatures(object):
    ...
    def __init__(self,
                 ...
                 start_position=None,
                 end_position=None):
        self.extra_sp_index = extra_sp_index
        self.qas_id = qas_id
        self.doc_tokens = doc_tokens
...
def read_examples(full_file):
    ...
    for case in tqdm(full_data):
        key = case['_id']
        qas_type = ""  # case['type']
        sup_facts = set([(sp[0], sp[1]) for sp in case['supporting_facts']])  # 证据+编号
        sup_titles = set([sp[0] for sp in case['supporting_facts']])  # 证据
        ...
        for paragraph in para_data:
            ...
            for local_sent_id, sent in enumerate(sents):
                ...
                if local_sent_name in sup_facts:  # 证据+编号
                ...
                if not JUDGE_FLAG and not FIND_FLAG and len(answer_offsets) > 0:  # len(answer_offsets) > 0表示找到了答案
                    FIND_FLAG = True
        ...
        #     print("sup fact id {}".format(sup_facts_sent_id))
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, max_query_length, max_answer_length, mode):
    # max_query_length = 50

    features = []
    failed = 0
    for (example_index, example) in enumerate(tqdm(examples)):
        if example.orig_answer_text == 'unknown':
            ans_type = 1
        elif example.orig_answer_text == 'yes':
            ans_type = 2
        ...
        if mode == "train":
            for token in example.orig_answer_text.split(' '):
                all_doc_tokens.extend(tokenizer.tokenize(token))
            if len(all_doc_tokens) > max_answer_length - 1:
                all_doc_tokens = all_doc_tokens[:max_answer_length - 1]
            all_doc_tokens.append("[SEP]")
        else:
            with open("./output/predictions.json", 'r', encoding='utf-8') as reader:
                full_data = json.load(reader)
            answers = full_data["answer"]

            for token in answers["{}".format(example.qas_id)].split(' '):
                all_doc_tokens.extend(tokenizer.tokenize(token))
            if len(all_doc_tokens) > max_answer_length - 1:
                all_doc_tokens = all_doc_tokens[:max_answer_length - 1]
            all_doc_tokens.append("[SEP]")
        for (i, token) in enumerate(example.doc_tokens):
            ...
            orig_to_tok_back_index.append(len(all_doc_tokens) - 1)  # 有可能是同一个词
        # if len(all_doc_tokens)>511:
        ...
        def relocate_tok_span(orig_start_position, orig_end_position, orig_text):
            ...
            return _improve_answer_span(
                all_doc_tokens, tok_start_position, tok_end_position, tokenizer, orig_text)  # 答案可能是切分词的一部分japanese
        ...
        extra_sp_doc_tokens = all_doc_tokens
        ...
        all_sentence_spans = sentence_spans
        ...
        # largest_valid_index = _largest_valid_index(all_sentence_spans, max_seq_length)
        # if largest_valid_index:
        #
        #     error = all_sentence_spans[largest_valid_index][0] - all_sentence_spans[5][0]
        #     sentence_spans = all_sentence_spans[:5] + all_sentence_spans[largest_valid_index:]
        #     all_doc_tokens = extra_sp_doc_tokens[:all_sentence_spans[5][0]] + extra_sp_doc_tokens[
        #                                                                       all_sentence_spans[largest_valid_index][
        #                                                                           0]:]
        #     all_doc_tokens = all_doc_tokens[:max_seq_length - 1] + ["[SEP]"]
        #     doc_input_ids = tokenizer.convert_tokens_to_ids(all_doc_tokens)
        #
        #     doc_input_mask = [1] * len(doc_input_ids)
        #     doc_segment_ids = [0] * len(query_tokens) + [1] * (len(doc_input_ids) - len(query_tokens))
        #     while len(doc_input_ids) < max_seq_length:
        #         doc_input_ids.append(0)
        #         doc_input_mask.append(0)
        #         doc_segment_ids.append(0)
        ...
        #     assert len(query_segment_ids) == max_query_length
        #
        #     sentence_spans = get_valid_spans2(sentence_spans, max_seq_length, error)
        #
        #     # sup_fact_ids = example.sup_fact_id
        #     # sent_num = len(sentence_spans)
        #     # sup_fact_ids = [sent_id for sent_id in sup_fact_ids if sent_id < sent_num]
        ...
        #
        #     features.append(
        #         InputFeatures(qas_id=example.qas_id + 100000,
        #                       doc_tokens=all_doc_tokens,
        #                       doc_input_ids=doc_input_ids,
        #                       doc_input_mask=doc_input_mask,
        #                       doc_segment_ids=doc_segment_ids,
        ...
        #                       token_to_orig_map=tok_to_orig_index,
        #                       start_position=ans_start_position,
        #                       extra_sp_index=largest_valid_index,
        #                       end_position=ans_end_position)
        #     )
    # print(len(features))
    return features
...
def get_valid_spans2(spans, limit, error):
    new_spans = []
    for span in spans[:5]:
        if span[1] < limit:
            new_spans.append(span)
    ...
    for span in spans[5:]:
        new_span = list(span)
        new_span[0] = new_span[0] - error
        new_span[1] = new_span[1] - error
...
if __name__ == '__main__':
    ...
    features = convert_examples_to_features(examples, tokenizer, max_seq_length=512, max_query_length=64, max_answer_length=64, mode=args.mode)
