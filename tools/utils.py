...
def convert_to_tokens(example, features, ids, y1, y2):
    answer_dict = dict()
    ...
    for i, qid in enumerate(ids):   # article id
        ...
        # print(y1[i])
        # print(y2[i])
        if y1[i] == 512 or y2[i] == 512:
            answer_text = 'unknown'
        elif y1[i] ==513 or y2[i] == 513:
            answer_text = 'yes'
        elif y1[i] ==514 or y2[i] == 514:
            answer_text = 'no'
        else:
            doc_tokens = features[qid].doc_tokens
            tok_tokens = doc_tokens[y1[i]: y2[i] + 1]
            tok_to_orig_map = features[qid].token_to_orig_map
            if y2[i] < len(tok_to_orig_map) and y1[i] <= y2[i]:
