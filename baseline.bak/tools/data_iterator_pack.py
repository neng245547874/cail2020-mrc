...
class DataIteratorPack(object):
    def __init__(self, features, example_dict,features_dict, bsz, device, sent_limit, entity_limit,
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for i in range(len(cur_batch)):
                case = cur_batch[i]
                extra_index = self.features_dict[case.qas_id].extra_sp_index
                query_length = len(case.query_tokens)
                ...
                if case.ans_type == 0:
                    if len(case.end_position) == 0:
                        ...
                        y1[i] = case.start_position[0]  # 取第一个匹配到的答案位置
                ...
                for j, sent_span in enumerate(case.sent_spans[:self.sent_limit]):
                    if case.qas_id > 1000000:
                        if j < 5:
                            is_sp_flag = j in case.sup_fact_ids
                        else:
                            is_sp_flag = j+extra_index-5 in case.sup_fact_ids
