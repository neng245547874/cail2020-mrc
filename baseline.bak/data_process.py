...
def convert_examples_to_features(examples, tokenizer, max_seq_length, max_query_length):
    ...
    for (example_index, example) in enumerate(tqdm(examples)):
        ...
        if largest_valid_index:
            extra_sp_index = largest_valid_index - 5
            if extra_sp_index < 0:
                # print(extra_sp_index)
                continue
            error = all_sentence_spans[extra_sp_index-1][0] - all_sentence_spans[0][0]
            sentence_spans = all_sentence_spans[extra_sp_index:]
            extra_index= sentence_spans[0][0]
            all_doc_tokens = extra_sp_doc_tokens[:len(query_tokens)] + extra_sp_doc_tokens[extra_index:]
            all_doc_tokens = all_doc_tokens[:max_seq_length - 1] + ["[SEP]"]
...
if __name__ == '__main__':
    ...
    features = convert_examples_to_features(examples, tokenizer, max_seq_length=512, max_query_length=64)
